...
class Section:
    def __init__(self, title: str, children: List[Union[Page, Section, Link]], lang: str) -> None:
        self.title = title
        self.lang = lang
...
def get_navigation(files: Files, config: Union[MkDocsConfig, Mapping[str, Any]]) -> Navigation:
    ...
    if config['theme'] is not None:
        if config['theme']['language'] is not None:
            default_language = config['theme']['language']

    items = []
    if default_language is not None:
        for lang_nav in nav_config:
            lang_str = list(lang_nav.keys())[0]
            lang_nav_list = lang_nav[lang_str]
            temp_list = _data_to_navigation(lang_nav_list, files, config, lang_str)
    ...
    if missing_from_config:
        ...
        for file in missing_from_config:
            Page(None, file, config, default_language)
...
def _data_to_navigation(data, files: Files, config: Union[MkDocsConfig, Mapping[str, Any]], lang: str):
    if isinstance(data, dict):
        return [
            _data_to_navigation((key, value), files, config, lang)
            if isinstance(value, str)
            else Section(title=key, children=_data_to_navigation(value, files, config, lang), lang=lang)
            for key, value in data.items()
        ]
    elif isinstance(data, list):
        return [
            _data_to_navigation(item, files, config, lang)[0]
            if isinstance(item, dict) and len(item) == 1
            else _data_to_navigation(item, files, config, lang)
            for item in data
        ]
    ...
    if file:
        return Page(title, file, config, lang)
